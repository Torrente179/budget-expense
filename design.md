...
| **Budget** | `/budget` | Dual engines: **Límites de gasto** (ceilings; 100%=exceeded) and **Metas de aportación** (floors; 100%=success). Hero remaining = income − spent. Plan distribution from planned amounts. Recommendation when a limit is over. Methods seed by `budget_role` + `kind`. Mobile: stacked hero → limits/goals → distribution. |
