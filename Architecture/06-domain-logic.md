...
[`src/lib/balance-checkpoint.ts`](../src/lib/balance-checkpoint.ts) — pure,
unit-tested checkpoint ordering and tracked-balance math
(`npm run test:balance`). Home's separate source-selection contract lives in
[`src/lib/home/month-cashflow.ts`](../src/lib/home/month-cashflow.ts) and is
tested by `npm run test:home`.
...
**2. Exact decimal aggregation.** The legacy JavaScript path converts rows to
cents, sums as integers, and divides back:
...
integer cents eliminates it. The primary `prepare_month_snapshot` RPC achieves
the same invariant with Postgres `NUMERIC` sums.
...
### Home carryover contract

The tracked calculation is continuous: the balance target is local today for
the current month, the selected month-end for a past month, and unavailable for
a future month. Checkpoint lookup is not constrained to the selected month. An
eligible July checkpoint plus all qualifying later movements therefore produces
August's real available cash without any rollover record.

Home deliberately resolves two independent layers:

```text
headline = trackedBalance ?? monthlyPlanRemainder ?? unavailable
plan pace = selectedMonthExpenses / selectedMonthIncome
```

`resolveHomeAvailableBalance` requires a finite tracked value, otherwise falls
back to `resolveMonthCashflow`'s month-only remainder. Its daily guide uses the
same selected headline, clamps negative cash to zero for recommendation
purposes, and never adds the plan remainder to tracked cash.

`BudgetSummaryHero` always stays on the monthly layer and labels its amount as
the remainder in this month's plan. Home's plan ring/bar also stays monthly,
even when the larger headline carries cash from a prior month.

See [`docs/balance-carryover.md`](../docs/balance-carryover.md) for the status
matrix, target-date rules, FX fallback, cache behavior, regression fixture, and
